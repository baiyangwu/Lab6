...
public class SudokuController implements Initializable {
	...
	private LatinSquare zeroTracker;
	...
	private void CreateSudokuInstance() {
		...
		int[][] a = new int[s.getLatinSquare().length][];
		
		for (int i = 0; i < s.getLatinSquare().length; i++) {
			a[i] = s.getLatinSquare()[i].clone();
		}
		zeroTracker = new LatinSquare(a);
	...
	private void BuildGrids() {
		...
		JOptionPane.showMessageDialog(null, "Welcome!\n Instructions: left click a number to move and right click a number to delete."
	...
	private GridPane BuildSudokuGrid() {
		...
		for (int iCol = 0; iCol < s.getiSize(); iCol++) {
			...
			for (int iRow = 0; iRow < s.getiSize(); iRow++) {
				...
				paneTarget.setOnMousePressed(e -> {
				...
				paneTarget.setOnDragDetected(new EventHandler<MouseEvent>() {
					public void handle(MouseEvent event) {
						Dragboard a = paneTarget.startDragAndDrop(TransferMode.ANY);
						ClipboardContent c = new ClipboardContent();
						c.put(myFormat, paneTarget.getCell());
						a.setContent(c);
						event.consume();
					}
				});
					...
					public void handle(DragEvent event) {
						...
						if (db.hasContent(myFormat)) {
							...
							if (!s.isValidValue(CellTo.getiRow(), CellTo.getiCol(), CellFrom.getiCellValue())) {
								...
								labelMistakes.setText("Mistakes: " + s.getMistakes());
								if (s.getMistakes() >= eGD.getMaxMistakes()) { 
									JOptionPane.showMessageDialog(null, "Mistakes reached to maximum.", "Game Over", JOptionPane.INFORMATION_MESSAGE);
									System.exit(0);
								}
							...
							if (!zeroTracker.ContainsZero()) { 
								s.setLatinSquare(zeroTracker.getLatinSquare());
								if (s.isSudoku()) {
